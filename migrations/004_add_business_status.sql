...
ALTER TABLE places
ADD COLUMN business_status VARCHAR(50) DEFAULT 'OPERATIONAL';

CREATE INDEX idx_places_business_status ON places(business_status);

COMMENT ON COLUMN places.business_status IS 'Business status from Google: OPERATIONAL, CLOSED_TEMPORARILY, CLOSED_PERMANENTLY';

-- Update view to include business_status
...
COMMENT ON VIEW activities IS 'Unified view of places and events for recommendations, includes Google Place IDs and business status';
